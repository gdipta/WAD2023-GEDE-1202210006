...
<body>
    ...
    $update = "SELECT * FROM showroom_mobil WHERE id_mobil = $id";
    $result = $koneksi->query($update);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "Nama Mobil: " . $row["nama_mobil"] . " - Brand: " . $row["brand_mobil"] . " - Warna: " . $row["warna_mobil"] . " - Tipe: " . $row["tipe_mobil"] . " - Harga: " . $row["harga_mobil"];
        }
    } else {
        echo "0 results";
    }
